...
import com.cargohub.cargoloader.CargoSorter.BoxChainedComparator;
import com.cargohub.cargoloader.CargoSorter.FragilityComparator;
import com.cargohub.cargoloader.CargoSorter.VolumeComparator;
...
public class LoadingServiceImpl {
	...
	public List<TransporterEntity> loadAllTransportersInHub(String hubName) {
		...
		// TODO: Work only with orders
		// TODO: check on splitting & splitting functionality
		// TODO: 1. Find all orders by longest route
		// TODO: 2. Find biggest route after sorting by volume
		// TODO: 3. If checkVolume and Weight < limits
		// TODO: 4. If free space >0 add particular routes
	...
	private CarrierCompartmentEntity loadCompartment(CarrierCompartmentEntity compartment,
			List<OrderEntity> maxVolumeList, List<OrderEntity> allOrders, double cellSize) {
	...
	private List<OrderEntity> optimizeOrdersForLoading(List<OrderEntity> maxVolumeList, List<OrderEntity> allOrders,
			CarrierCompartmentEntity compartment) {
		...
		if (!(Math.abs(compartmentVolume - ordersVolume) < accuracy && compartmentMaxWeight >= ordersWeight)) {
			...
			for (OrderEntity particularOrder : particularRouteOrders) {
				...
				if (Math.abs(compartmentVolume - newOrdersVolume) < accuracy
						&& compartmentMaxWeight >= newOrdersWeight) {
